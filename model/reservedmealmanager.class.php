...
class ReservedMealManager {
	...
	public function addReservedMeal (ReservedMeal $new) {
		$requete = $this->db->prepare('INSERT INTO reservedmeal (idReservation, idPlat, quantity) 
			VALUES (:idReservation, :idPlat, :quantity)');

		$idReservation = $new->getIdReservation();
		$idPlat = $new->getIdPlat();
		$quantity = $new->getQuantity();
		$requete->bindValue(':idReservation', $idReservation);
		$requete->bindValue(':idPlat', $idPlat);
